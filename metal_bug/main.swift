...
private enum PipelineMode: CaseIterable {
    case baseline
    case bugTrigger
...
private func runVariant(device: MTLDevice,
    ...
    let pipeline = try makePipeline(device: device, function: function, mode: mode)
...
private func main() throws {
    ...
    print("=== Metal Pipeline Descriptor Barrier Bug Repro ===")
    ...
    let pipelineModes = PipelineMode.allCases.map { $0.title }.joined(separator: ", ")
    print("Kernel function: \(config.functionName)")
    print("Discovered dumped shaders: \(variants.count)")
    print("Pipeline modes: \(pipelineModes)")
    ...
    for variant in variants {
        for mode in PipelineMode.allCases {
            print("--- Test \(testIndex): \(variant.title) / \(mode.title) ---")
